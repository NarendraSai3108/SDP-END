...
const AdminManagersPage = () => {
  ...
  const addManager = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${config.BACKEND_URL}/api/admin/addManager`, newManager);
      setNewManager({ name: "", email: "", password: "" }); // reset form
  ...
  return (
    <>
      <style>{`
        ...
        .input-focus:focus { border-color: #06b6d4; }
        .table-row:hover { background-color: #f8fafc; }
        .add-button:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(6, 182, 212, 0.3); }
      ...
      <div style={styles.container} className="fade-in">
        ...
        <div style={styles.statsCards} className="stats-responsive">
          <div style={styles.statCard}>
            <div style={styles.statNumber}>{managers.length}</div>
            <div style={styles.statLabel}>Total Managers</div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statNumber}>{managers.filter(m => m.approved).length}</div>
        ...
        <div style={styles.tableContainer}>
          {loading ? (
            ...
            <div style={styles.emptyState}>
              ...
              <div>
                {searchTerm ? 'Try adjusting your search criteria' : 'No managers have been added yet'}
              </div>
            </div>
          ) : (
            <table style={styles.table} className="table-responsive">
              ...
              <tbody>
                {filteredManagers.map((manager) => (
                  <tr key={manager.id} style={styles.tableRow} className="table-row">
                    ...
                    <td style={styles.tableCell} className="table-cell-responsive">
                      <span style={{
                        ...styles.statusBadge,
                        ...(manager.approved ? styles.approvedBadge : styles.pendingBadge)
                      }}>
        ...
        {showAddModal && (
          <div style={styles.modal}>
            <div style={styles.modalContent} className="modal-responsive">
              <h2 style={styles.modalTitle}>Add New Manager</h2>
